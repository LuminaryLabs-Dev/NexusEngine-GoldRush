...
- Cloud-side raw asset/audio copies must now include the full receipt set in `docs/cloud-asset-receipts.md` and pass `tools/validation/validate-cloud-asset-receipts.mjs`. Before cloud import this gate must report `waiting-for-cloud-asset-receipts`; once any raw candidate or receipt appears, source discovery, deny scan, secret scan, copy ledger, hashes, and classification receipts are all mandatory.
