# Combat encounter readability gap

Status: open
Audit wave: 2026-07-01-wave-001
Severity: high
Domain: combat/world/gameplay
Owning kit or workspace: n:gameplay:combat-pressure plus n:goldrush:ambush-pressure
Roadmap rows informed: 032, 071, 072, 073, 074, 075, 076, 077

## Problem

Combat can be structurally represented but still not playable if threats, cover, hits, damage, revive, weapons, and extraction interruption do not read in the world.
...
- Threat approach, cover choice, weapon feedback, hit confirmation, and escape/extract decision are visible from the third-person camera.
- Combat receipts correspond to natural aim/move/cover actions, not direct setup helpers.
- Final pressure creates convergence without erasing extraction strategy.

## Edge Cases To Hunt

- Threat spawns behind unreadable terrain.
- Cover looks solid but physics/query says it is not cover.
- Hit feedback is invisible at long range.
- Solo staging gets stuck in downed/revive rules designed for squads.
- Zone pressure and extraction timer conflict unfairly.

## Deployment Issues To Hunt

- Network delay may make hit/cover state look unfair.
- Low frame rate can hide telegraph frames.
...
Build combat around authored cover and readable pressure before expanding weapon count.
