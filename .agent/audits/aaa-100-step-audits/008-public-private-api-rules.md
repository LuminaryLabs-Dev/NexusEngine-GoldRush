# Audit 008 - Public and private API rules

Status: docs-only audit planned
Roadmap packet: .agent/research/aaa-100/008-public-private-api-rules.md
Simulation packet: .agent/simulations/aaa-100/008-public-private-api-rules.md
Domain: architecture
...
Audit gate: Public API does not become config sprawl

## Intention Check

This audit exists to stop **Public/private API rules** from becoming a shallow proof or isolated feature. The step must make the long-term GoldRush target more true: high-fidelity toon-shaded wild-west extraction battle royale, 60-player-ready architecture, single-player staging, and kit-owned gameplay from title to results.
...
- Ownership risk: domain service architecture can drift if implementation lands in the app host or renderer for convenience.
...
- Roadmap packet reviewed: .agent/research/aaa-100/008-public-private-api-rules.md
- Simulation packet reviewed: .agent/simulations/aaa-100/008-public-private-api-rules.md
- Validator or proof named before implementation.
- Change log updated after any meaningful packet or repo change.
- Matrix row updated only when evidence matches the stated gate: Public API does not become config sprawl
...
Not complete. This audit should be closed only after the current implementation, proof coverage, deploy state, and player-view evidence all satisfy **Public API does not become config sprawl** without weakening the original AAA goal.
