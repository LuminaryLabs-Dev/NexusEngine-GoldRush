# Audit 071 - Combat mechanics

Status: docs-only audit planned
Roadmap packet: .agent/research/aaa-100/071-combat-mechanics.md
Simulation packet: .agent/simulations/aaa-100/071-combat-mechanics.md
Domain: combat
...
This audit exists to stop **Combat mechanics** from becoming a shallow proof or isolated feature. The step must make the long-term GoldRush target more true: high-fidelity toon-shaded wild-west extraction battle royale, 60-player-ready architecture, single-player staging, and kit-owned gameplay from title to results.
...
- Ownership risk: combat pressure and counterplay can drift if implementation lands in the app host or renderer for convenience.
...
- Roadmap packet reviewed: .agent/research/aaa-100/071-combat-mechanics.md
- Simulation packet reviewed: .agent/simulations/aaa-100/071-combat-mechanics.md
- Validator or proof named before implementation.
- Change log updated after any meaningful packet or repo change.
- Matrix row updated only when evidence matches the stated gate: Combat is playable, not just receipts

## Edge Cases

- Restart after partial implementation selects an older packet and duplicates work.
- A newer kit supersedes n:goldrush:ambush-pressure, but this packet still points at the old owner.
...
Not complete. This audit should be closed only after the current implementation, proof coverage, deploy state, and player-view evidence all satisfy **Combat is playable, not just receipts** without weakening the original AAA goal.
