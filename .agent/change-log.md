...
- 2026-08-28 UTC - Replaced the bounded terrain implementation with deterministic `goldrush-seeded-world-r1`: a 10.24-kilometer world, 128-meter camera-centered tiles, four concentric LOD rings, a local shared-source collider, stable tile-owned features, and the authored basin blended at the origin. Refined canyon framing, destination silhouettes, and route width through five passing Lavapipe video review iterations; `CI=true npm run check`, production build, and exact Pages-route extraction proof passed. Non-CI check remains blocked only at the intentionally required absent external NexusSimulator discovery gate.
