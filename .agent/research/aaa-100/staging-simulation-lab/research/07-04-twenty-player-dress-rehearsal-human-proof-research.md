# Twenty Player Dress Rehearsal - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/07-04-twenty-player-dress-rehearsal-human-proof.md
...
The owner kit is `n:goldrush:twenty-player-dress-rehearsal`. It should expose a small public API, use private helpers for setup and measurement, emit `staging.twenty.roster.ready`, and expose `twentyPlayerDressRehearsal` for validation.
...
Use `validate-twenty-player-dress-rehearsal` as the first validator seed. Negative fixtures must fail the fakeout: A 20-player count exists but no update budget, event volume, or visible pressure is measured.
