# Single Player Practice Loop - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/02-04-single-player-practice-loop-human-proof.md
...
The owner kit is `n:goldrush:single-player-practice`. It should expose a small public API, use private helpers for setup and measurement, emit `staging.practice.loop.started`, and expose `singlePlayerPracticeLoop` for validation.
...
Use `validate-single-player-practice-loop` as the first validator seed. Negative fixtures must fail the fakeout: A solo route reaches results only through direct completion helpers or teleports.
