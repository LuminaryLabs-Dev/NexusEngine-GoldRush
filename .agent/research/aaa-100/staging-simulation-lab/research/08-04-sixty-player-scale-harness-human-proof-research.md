# Sixty Player Scale Harness - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/08-04-sixty-player-scale-harness-human-proof.md
...
The owner kit is `n:goldrush:sixty-player-scale-harness`. It should expose a small public API, use private helpers for setup and measurement, emit `staging.sixty.scale.sampled`, and expose `sixtyPlayerScaleHarness` for validation.
...
Use `validate-sixty-player-scale-harness` as the first validator seed. Negative fixtures must fail the fakeout: A single-browser proof is described as live 60-player readiness.
