# Scenario Seed Library - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/10-04-scenario-seed-library-human-proof.md
...
The owner kit is `n:goldrush:scenario-seed-library`. It should expose a small public API, use private helpers for setup and measurement, emit `scenario.seed.loaded`, and expose `scenarioSeedLibrary` for validation.
...
Use `validate-scenario-seed-library` as the first validator seed. Negative fixtures must fail the fakeout: Tests are deterministic only because hidden globals or local browser state persist.
