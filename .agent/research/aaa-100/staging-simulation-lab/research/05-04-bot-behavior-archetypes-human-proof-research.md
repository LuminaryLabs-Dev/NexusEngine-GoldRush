# Bot Behavior Archetypes - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/05-04-bot-behavior-archetypes-human-proof.md
...
The owner kit is `n:goldrush:bot-behavior-archetypes`. It should expose a small public API, use private helpers for setup and measurement, emit `bot.behavior.intent.changed`, and expose `botBehaviorArchetypes` for validation.
...
Use `validate-bot-behavior-archetypes` as the first validator seed. Negative fixtures must fail the fakeout: Bots exist but only stand still, chase blindly, or deal hidden damage.
