# Dummy Squad Simulation - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/06-04-dummy-squad-simulation-human-proof.md
...
The owner kit is `n:goldrush:dummy-squad-simulation`. It should expose a small public API, use private helpers for setup and measurement, emit `staging.squad.state.changed`, and expose `dummySquadSimulation` for validation.
...
Use `validate-dummy-squad-simulation` as the first validator seed. Negative fixtures must fail the fakeout: The report says squad play but every entity acts as isolated singletons.
