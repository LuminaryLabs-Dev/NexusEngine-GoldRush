# Training Yard Onboarding - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/03-04-training-yard-onboarding-human-proof.md
...
The owner kit is `n:goldrush:training-yard-onboarding`. It should expose a small public API, use private helpers for setup and measurement, emit `training.objective.completed`, and expose `trainingYardOnboarding` for validation.
...
Use `validate-training-yard-onboarding` as the first validator seed. Negative fixtures must fail the fakeout: Tutorial text exists but the player never performs the actual controls in-world.
