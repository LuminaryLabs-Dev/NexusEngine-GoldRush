# Bot Roster Seeding - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/04-04-bot-roster-seeding-human-proof.md
...
The owner kit is `n:goldrush:bot-roster-seeding`. It should expose a small public API, use private helpers for setup and measurement, emit `bot.roster.seeded`, and expose `botRosterSeeding` for validation.
...
Use `validate-bot-roster-seeding` as the first validator seed. Negative fixtures must fail the fakeout: Bot count is a number in state but bots have no spawn, team, behavior, or receipt identity.
