# Room Partition Load Test - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/09-04-room-partition-load-test-human-proof.md
...
The owner kit is `n:goldrush:room-partition-load-test`. It should expose a small public API, use private helpers for setup and measurement, emit `network.partition.load.sampled`, and expose `roomPartitionLoadTest` for validation.
...
Use `validate-room-partition-load-test` as the first validator seed. Negative fixtures must fail the fakeout: Partition math passes but player handoff, reconnect, or party identity is untested.
