# Staging Mode Registry - Human Proof Research

Status: planned docs-only
Related packet: ../scenarios/01-04-staging-mode-registry-human-proof.md
...
The owner kit is `n:goldrush:staging-mode-registry`. It should expose a small public API, use private helpers for setup and measurement, emit `staging.mode.selected`, and expose `stagingModeRegistry` for validation.
...
Use `validate-staging-mode-registry` as the first validator seed. Negative fixtures must fail the fakeout: A hidden debug flag changes behavior but no player, tester, or validator can see which mode is active.
