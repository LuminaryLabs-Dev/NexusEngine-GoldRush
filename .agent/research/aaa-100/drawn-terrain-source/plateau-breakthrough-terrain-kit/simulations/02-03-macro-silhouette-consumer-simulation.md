# Macro Silhouette Consumer Implementation Simulation
...
A future implementation adds macro-silhouette / consumer to the terrain source stack. The first version works locally because one consumer reads the new field, but the broader map still risks plateauing if the other consumers keep their old local assumptions.
