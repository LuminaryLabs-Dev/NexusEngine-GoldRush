...
Document the source signals and design implications for `railRevisionPolicy` before future implementation.
...
What exact consumer echo proves `railRevisionPolicy` is source-owned and not inferred from a mesh, cinematic helper, timer, or local train curve?
