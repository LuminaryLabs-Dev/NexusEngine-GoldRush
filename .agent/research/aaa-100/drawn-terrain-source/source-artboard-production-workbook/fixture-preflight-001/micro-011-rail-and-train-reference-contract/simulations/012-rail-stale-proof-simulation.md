# 012 - Rail Stale Proof Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `railRevisionPolicy`

## Simulated Implementation

1. Add `railRevisionPolicy` to the tiny source fixture.
...
- Keep `railRevisionPolicy` as a source-data concern only.
...
Future implementation is acceptable only if source revision changes mark rail meshes, train motion, boarding locks, camera handoff, audio cues, screenshots, simulator proof, and public proof stale. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
