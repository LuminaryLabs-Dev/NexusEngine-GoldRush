# 001 - Rail Spline Schema Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `railSplines`

## Simulated Implementation

1. Add `railSplines` to the tiny source fixture.
...
- Keep `railSplines` as a source-data concern only.
...
Future implementation is acceptable only if validator proves source-owned rail splines have id, control points, tangent policy, direction labels, grade bands, speed bands, segment ids, and revision. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
