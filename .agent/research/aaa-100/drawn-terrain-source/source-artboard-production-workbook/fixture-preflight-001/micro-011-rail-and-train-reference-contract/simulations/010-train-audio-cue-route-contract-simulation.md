# 010 - Train Audio Cue Route Contract Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `trainAudioRouteCues`

## Simulated Implementation

1. Add `trainAudioRouteCues` to the tiny source fixture.
...
- Keep `trainAudioRouteCues` as a source-data concern only.
...
Future implementation is acceptable only if validator proves train audio cues bind to rail segment id, distance band, motion state, door side, cue id, volume policy, and revision. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
