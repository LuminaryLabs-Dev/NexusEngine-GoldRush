# 008 - Rail Prop Placement Parity Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `railPropPlacementEcho`

## Simulated Implementation

1. Add `railPropPlacementEcho` to the tiny source fixture.
...
- Keep `railPropPlacementEcho` as a source-data concern only.
...
Future implementation is acceptable only if renderer snapshots echo rail anchor ids, raycast hits, prop family ids, spacing policy, side label, and fixture revision. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
