# 009 - Train Camera Follow Contract Simulation
...
1. Add `trainCameraRailHandoff` to the tiny source fixture.
...
- Keep `trainCameraRailHandoff` as a source-data concern only.
...
Future implementation is acceptable only if validator proves camera follow, over-shoulder return, train ride framing, player lock state, rail sample target, and revision are reported together. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
