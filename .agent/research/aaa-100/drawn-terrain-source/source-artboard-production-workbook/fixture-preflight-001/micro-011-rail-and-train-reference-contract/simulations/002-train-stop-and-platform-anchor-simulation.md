# 002 - Train Stop And Platform Anchor Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `trainStopAnchors`

## Simulated Implementation

1. Add `trainStopAnchors` to the tiny source fixture.
...
- Keep `trainStopAnchors` as a source-data concern only.
...
Future implementation is acceptable only if validator proves train stop anchors have id, position, facing, platform side, door side, approach radius, boarding range, and revision. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
