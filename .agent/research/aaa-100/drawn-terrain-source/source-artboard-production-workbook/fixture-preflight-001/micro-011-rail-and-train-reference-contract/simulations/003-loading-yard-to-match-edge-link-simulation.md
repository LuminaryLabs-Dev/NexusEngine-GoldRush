# 003 - Loading Yard To Match Edge Link Simulation

Status: planned docs-only
Parent atom: `011-rail-and-train-reference-contract`
Source field: `loadingYardMapEdgeLinks`

## Simulated Implementation

1. Add `loadingYardMapEdgeLinks` to the tiny source fixture.
...
- Keep `loadingYardMapEdgeLinks` as a source-data concern only.
...
Future implementation is acceptable only if validator proves loading scene and match map use shared rail edge ids, direction labels, handoff markers, and fixture revision. and the stop condition cannot pass through helper-only, scene-only, renderer-only, camera-only, audio-only, time-only, or stale-proof state.
