# 011 - Slope Negative Fixture Cases Research
...
What external and local architecture signals constrain `slopeNegativeCases` before implementation?
...
- Terrain raycast owns query output for `slopeNegativeCases`.
...
- Validator needs one negative case where validation only checks that slope exists and misses broken terrain-footing data.
