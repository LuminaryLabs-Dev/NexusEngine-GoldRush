# 011 - Slope Negative Fixture Cases Audit
...
The normal and slope pass can fake completion if `slopeNegativeCases` exists but consumers do not use it.

## Why It Matters

validation only checks that slope exists and misses broken terrain-footing data.
...
Do not mark this micro-step resolved until validator fails missing normals, non-unit normals, impossible slopes, and class-threshold contradictions and the validator catches the opposite failure.
