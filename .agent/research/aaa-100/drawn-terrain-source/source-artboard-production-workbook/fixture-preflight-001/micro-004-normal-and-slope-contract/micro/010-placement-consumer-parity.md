...
Make `placementNormalEcho` small enough for a future implementation pass.

## Source Field

- Required field: `placementNormalEcho`.
...
- Fail when `placementNormalEcho` is missing, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
prop placement snapshot echoes source normal and slope used for alignment or rejection.

## Stop Condition

Stop if this micro-step can pass while rocks, plants, mines, and cashout props look grounded but are aligned by renderer fallback.
