# 007 - Gradient Sample Neighborhood
...
Make `gradientKernel` small enough for a future implementation pass.

## Source Field

- Required field: `gradientKernel`.
...
- Fail when `gradientKernel` is missing, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
height-gradient normals declare sample neighborhood and edge behavior.

## Stop Condition

Stop if this micro-step can pass while slope flips or pulses across cells because each consumer samples a different neighborhood.
