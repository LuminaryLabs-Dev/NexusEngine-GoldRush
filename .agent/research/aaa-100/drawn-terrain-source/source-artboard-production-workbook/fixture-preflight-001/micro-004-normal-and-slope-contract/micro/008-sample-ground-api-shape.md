...
Make `sampleGroundApi` small enough for a future implementation pass.

## Source Field

- Required field: `sampleGroundApi`.
...
- Fail when `sampleGroundApi` is missing, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
Stop if this micro-step can pass while height, normal, and slope are queried separately and drift between systems.
