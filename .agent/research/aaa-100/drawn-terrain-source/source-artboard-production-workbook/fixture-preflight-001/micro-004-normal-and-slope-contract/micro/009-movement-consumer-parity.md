...
Make `movementGroundEcho` small enough for a future implementation pass.

## Source Field

- Required field: `movementGroundEcho`.
...
- Fail when `movementGroundEcho` is missing, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
movement snapshot echoes source normal and slope used for grounded, slow, slide, or blocked state.

## Stop Condition

Stop if this micro-step can pass while camera and character feel stable in proof while movement still uses hidden slope logic.
