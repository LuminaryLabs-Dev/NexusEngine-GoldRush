...
Make `walkableSlopeThresholds` small enough for a future implementation pass.

## Source Field

- Required field: `walkableSlopeThresholds`.
...
- Fail when `walkableSlopeThresholds` is missing, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
Stop if this micro-step can pass while the player can climb mountains or gets blocked on gentle terrain because thresholds are duplicated.
