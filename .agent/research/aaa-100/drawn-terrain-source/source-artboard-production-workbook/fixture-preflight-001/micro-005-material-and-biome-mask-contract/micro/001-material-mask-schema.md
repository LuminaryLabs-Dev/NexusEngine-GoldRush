...
Make `materialMask` small enough for a future implementation pass.

## Source Field

- Required field: `materialMask`.
...
- Fail when `materialMask` is missing, unknown, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
Stop if this micro-step can pass while renderer fills missing surface identity with a default color while gameplay and audio know nothing about it.
