# 011 - Mask Negative Fixture Cases
...
Make `maskNegativeCases` small enough for a future implementation pass.

## Source Field

- Required field: `maskNegativeCases`.
...
- Fail when `maskNegativeCases` is missing, unknown, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
validator fails missing masks, unknown tags, invalid weights, ambiguous dominant layers, and consumer tag drift.

## Stop Condition

Stop if this micro-step can pass while validation only checks that a mask exists and misses broken material or biome contracts.
