...
Make `maskWeights` small enough for a future implementation pass.

## Source Field

- Required field: `maskWeights`.
...
- Fail when `maskWeights` is missing, unknown, non-finite, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
validator rejects non-finite, negative, over-one, and under-specified blend weights.

## Stop Condition

Stop if this micro-step can pass while layer blending hides invalid masks and consumers disagree about the dominant terrain surface.
