# 011 - Mask Negative Fixture Cases Research
...
What external and local architecture signals constrain `maskNegativeCases` before implementation?
...
- Terrain material mask owns query output for `maskNegativeCases`.
...
- Validator needs one negative case where validation only checks that a mask exists and misses broken material or biome contracts.
