# 001 - Source Id And Revision

Status: planned docs-only
Fixture id: `goldrush.desert.artboard.fixture.001`
Domain: world/runtime
Generic kit candidate: `n:world:authored-terrain-mesh`
GoldRush kit candidate: `n:goldrush:desert-world-map`

## Purpose

Make one source-fixture concern small enough for a future implementation pass.

## Atomic Scope

- Source data: fixtureId, revisionId, revisionReason.
- Public proof: all consumers echo the same fixtureId and revisionId.
- Consumer boundary: the owning kit reports fixture id and revision id in snapshot output.
- Runtime boundary: no renderer, physics, placement, or gameplay fallback may replace this source field silently.

## Substeps

1. Define the source field or snapshot requirement.
2. Add a validator failure case for missing or drifting data.
3. Add one consumer proof path.
4. Add one human-view or state-proof expectation.
5. Mark stale proof if the fixture revision changes.

## Stop Condition

Stop if this atom can pass without proving all consumers echo the same fixtureId and revisionId.

## Micro Runway

Start future implementation with `../micro-001-source-id-and-revision/README.md`. That packet splits this atom into fixture id format, revision id format, revision reason, source hash inputs, authoring metadata, consumer echo registry, negative cases, summary snapshot, identity events, stale-proof flags, and restart packet linkage.
