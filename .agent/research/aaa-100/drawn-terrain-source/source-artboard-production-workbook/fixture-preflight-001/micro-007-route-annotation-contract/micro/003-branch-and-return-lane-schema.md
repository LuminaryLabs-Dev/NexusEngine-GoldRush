# 003 - Branch And Return Lane Schema
...
Make `branchReturnLanes` small enough for a future implementation pass.

## Source Field

- Required field: `branchReturnLanes`.
...
- Fail when `branchReturnLanes` is missing, disconnected, unknown, contradictory, unversioned, non-source-owned, or silently inferred by a consumer.
...
validator proves branch lanes and return lanes connect route segments to mine, town, rail, cashout, and cover approach points.

## Stop Condition

Stop if objective approach and retreat paths are not represented as source-owned lanes.
