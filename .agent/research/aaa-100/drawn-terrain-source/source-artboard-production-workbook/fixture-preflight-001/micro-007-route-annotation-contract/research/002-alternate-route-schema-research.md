# 002 - Alternate Route Schema Research
...
What external and local architecture signals constrain `alternateRoutes` before implementation?
...
- Route annotations own query output for `alternateRoutes`.
...
The likely fake-completion path is describing alternate routes in docs while route query data exposes only the primary path.
