# 004 - Route Node And Segment Id Research
...
What external and local architecture signals constrain `routeNodesAndSegments` before implementation?
...
- Route annotations own query output for `routeNodesAndSegments`.
...
The likely fake-completion path is coordinate-only snapshots that cannot compare local and public route revisions.
