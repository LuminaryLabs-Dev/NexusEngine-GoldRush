# 012 - Cover Pressure Stale Proof Simulation

Status: planned docs-only
Parent atom: `009-cover-and-pressure-annotation-contract`
Source field: `coverPressureRevisionPolicy`

## Simulated Implementation

1. Add `coverPressureRevisionPolicy` to the tiny source fixture.
...
- Keep `coverPressureRevisionPolicy` as a source-data concern only.
...
Future implementation is acceptable only if source revision changes mark renderer telegraphs, combat routing, ambush receipts, screenshots, simulator proof, and public proof stale and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
