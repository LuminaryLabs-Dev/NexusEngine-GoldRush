# 008 - Pressure Query API Shape Simulation

Status: planned docs-only
Parent atom: `009-cover-and-pressure-annotation-contract`
Source field: `pressureQueryApi`

## Simulated Implementation

1. Add `pressureQueryApi` to the tiny source fixture.
...
- Keep `pressureQueryApi` as a source-data concern only.
...
Future implementation is acceptable only if getPressureAt reports cover id, threat lane id, seed id, risk tier, counterplay id, and revision at named proof points and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
