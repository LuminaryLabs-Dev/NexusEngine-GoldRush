# 002 - Threat Lane Schema Simulation
...
1. Add `threatLanes` to the tiny source fixture.
...
- Keep `threatLanes` as a source-data concern only.
...
Future implementation is acceptable only if validator proves threat lanes have id, origin band, target band, width, visibility class, risk tier, and revision and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
