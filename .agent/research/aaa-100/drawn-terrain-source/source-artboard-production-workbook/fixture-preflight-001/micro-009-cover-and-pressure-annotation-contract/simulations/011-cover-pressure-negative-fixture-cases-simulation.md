# 011 - Cover Pressure Negative Fixture Cases Simulation

Status: planned docs-only
Parent atom: `009-cover-and-pressure-annotation-contract`
Source field: `coverPressureNegativeCases`

## Simulated Implementation

1. Add `coverPressureNegativeCases` to the tiny source fixture.
...
- Keep `coverPressureNegativeCases` as a source-data concern only.
...
Future implementation is acceptable only if validator fails missing cover ids, duplicate lane ids, orphan pressure seeds, unknown tags, unreachable counterplay, and stale consumer echoes and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
