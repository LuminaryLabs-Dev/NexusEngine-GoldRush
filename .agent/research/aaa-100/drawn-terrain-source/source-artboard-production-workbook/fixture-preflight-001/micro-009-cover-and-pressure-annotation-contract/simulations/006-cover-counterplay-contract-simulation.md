# 006 - Cover Counterplay Contract Simulation

Status: planned docs-only
Parent atom: `009-cover-and-pressure-annotation-contract`
Source field: `coverCounterplay`

## Simulated Implementation

1. Add `coverCounterplay` to the tiny source fixture.
...
- Keep `coverCounterplay` as a source-data concern only.
...
Future implementation is acceptable only if validator proves each threat lane has at least one reachable counterplay pocket, flank, retreat, or low-exposure route and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
