# 004 - Sightline And Occlusion Tags Simulation
...
1. Add `sightlineOcclusionTags` to the tiny source fixture.
...
- Keep `sightlineOcclusionTags` as a source-data concern only.
...
Future implementation is acceptable only if validator proves cover and threat lanes expose line-of-sight, occlusion, elevation, peek, and flank tags as closed source-owned values and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
