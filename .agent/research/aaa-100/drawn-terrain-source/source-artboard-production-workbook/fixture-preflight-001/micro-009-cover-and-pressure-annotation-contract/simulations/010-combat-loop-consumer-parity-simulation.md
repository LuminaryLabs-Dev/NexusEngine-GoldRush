# 010 - Combat Loop Consumer Parity Simulation
...
1. Add `combatLoopPressureEcho` to the tiny source fixture.
...
- Keep `combatLoopPressureEcho` as a source-data concern only.
...
Future implementation is acceptable only if ambush pressure, combat route guidance, action surface, receipts, replay, and results name the same source pressure ids and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
