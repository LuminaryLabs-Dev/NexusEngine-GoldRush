# 007 - Combat Proof Point Contract Simulation

Status: planned docs-only
Parent atom: `009-cover-and-pressure-annotation-contract`
Source field: `combatProofPoints`

## Simulated Implementation

1. Add `combatProofPoints` to the tiny source fixture.
...
- Keep `combatProofPoints` as a source-data concern only.
...
Future implementation is acceptable only if fixture names proof points for safe approach, exposed crossing, cover entry, peek, flank, ambush trigger, retreat, and off-route failure and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
