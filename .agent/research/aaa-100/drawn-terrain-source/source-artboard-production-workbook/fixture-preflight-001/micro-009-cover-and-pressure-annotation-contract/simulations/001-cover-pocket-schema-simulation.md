# 001 - Cover Pocket Schema Simulation
...
1. Add `coverPockets` to the tiny source fixture.
...
- Keep `coverPockets` as a source-data concern only.
...
Future implementation is acceptable only if validator proves source-owned cover pockets have id, shape, stance class, protection direction, exposure class, and revision and the stop condition cannot pass through helper-only, combat-only, or renderer-only state.
