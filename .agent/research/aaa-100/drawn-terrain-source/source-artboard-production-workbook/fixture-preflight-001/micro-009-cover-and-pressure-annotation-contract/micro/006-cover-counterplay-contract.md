# 006 - Cover Counterplay Contract
...
Make `coverCounterplay` small enough for a future implementation pass.

## Source Field

- Required field: `coverCounterplay`.
...
- Fail when `coverCounterplay` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
validator proves each threat lane has at least one reachable counterplay pocket, flank, retreat, or low-exposure route.

## Stop Condition

Stop if pressure can create a no-read, no-cover, no-retreat fail state.
