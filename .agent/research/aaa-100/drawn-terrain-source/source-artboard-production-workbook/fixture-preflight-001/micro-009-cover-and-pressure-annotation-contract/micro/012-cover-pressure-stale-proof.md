# 012 - Cover Pressure Stale Proof
...
Make `coverPressureRevisionPolicy` small enough for a future implementation pass.

## Source Field

- Required field: `coverPressureRevisionPolicy`.
...
- Fail when `coverPressureRevisionPolicy` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
source revision changes mark renderer telegraphs, combat routing, ambush receipts, screenshots, simulator proof, and public proof stale.

## Stop Condition

Stop if source cover or pressure changes do not force combat proof refresh.
