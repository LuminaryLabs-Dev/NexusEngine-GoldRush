...
Make `pressureSeeds` small enough for a future implementation pass.

## Source Field

- Required field: `pressureSeeds`.
...
- Fail when `pressureSeeds` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
validator proves pressure seeds have id, trigger tags, intensity range, radius, route relation, and revision.

## Stop Condition

Stop if pressure intensity is only derived from cargo amount or timer without map-authored pressure context.
