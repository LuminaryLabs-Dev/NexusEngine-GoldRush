...
Make `coverPockets` small enough for a future implementation pass.

## Source Field

- Required field: `coverPockets`.
...
- Fail when `coverPockets` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
