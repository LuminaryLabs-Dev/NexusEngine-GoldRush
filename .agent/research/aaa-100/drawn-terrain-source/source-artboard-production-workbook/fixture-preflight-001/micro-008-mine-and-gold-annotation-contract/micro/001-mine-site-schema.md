...
Make `mineSites` small enough for a future implementation pass.

## Source Field

- Required field: `mineSites`.
...
- Fail when `mineSites` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, or silently replaced by a consumer.
...
validator proves at least one source-owned mine site with stable id, bounds, approach zone, entry marker, revision, and consumer echo.

## Stop Condition

Stop if a mine entrance can be derived only from a mesh name, renderer group, or hardcoded coordinate.
