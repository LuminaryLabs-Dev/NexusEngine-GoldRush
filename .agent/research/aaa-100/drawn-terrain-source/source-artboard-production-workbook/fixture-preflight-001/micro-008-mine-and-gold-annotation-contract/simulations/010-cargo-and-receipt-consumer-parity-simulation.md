# 010 - Cargo And Receipt Consumer Parity Simulation
...
1. Add `cargoReceiptEcho` to the tiny source fixture.
...
- Keep `cargoReceiptEcho` as a source-data concern only.
...
Future implementation is acceptable only if cargo, receipts, score, replay, and results name the same source mine or seam annotation that produced the gold and the stop condition cannot pass through helper-only or renderer-only state.
