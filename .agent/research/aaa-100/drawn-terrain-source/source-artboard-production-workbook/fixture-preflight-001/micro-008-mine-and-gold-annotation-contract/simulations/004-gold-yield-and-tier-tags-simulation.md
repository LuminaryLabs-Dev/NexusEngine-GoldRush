# 004 - Gold Yield And Tier Tags Simulation
...
1. Add `goldYieldTierTags` to the tiny source fixture.
...
- Keep `goldYieldTierTags` as a source-data concern only.
...
Future implementation is acceptable only if validator proves yield tiers are closed values attached to source annotations and echoed by mining, cargo, and scoring consumers and the stop condition cannot pass through helper-only or renderer-only state.
