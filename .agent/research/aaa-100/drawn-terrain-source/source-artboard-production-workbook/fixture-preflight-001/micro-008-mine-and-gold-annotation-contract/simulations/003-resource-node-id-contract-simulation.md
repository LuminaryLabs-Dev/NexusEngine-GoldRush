# 003 - Resource Node Id Contract Simulation
...
1. Add `resourceNodeIds` to the tiny source fixture.
...
- Keep `resourceNodeIds` as a source-data concern only.
...
Future implementation is acceptable only if validator proves every mineable node has a stable source id, annotation id, node id, and revision echo and the stop condition cannot pass through helper-only or renderer-only state.
