# 009 - Mine Hold Action Consumer Parity Simulation
...
1. Add `mineHoldActionEcho` to the tiny source fixture.
...
- Keep `mineHoldActionEcho` as a source-data concern only.
...
Future implementation is acceptable only if interaction-hold snapshots echo source annotation id, node id, hold progress, cancel reason, and revision and the stop condition cannot pass through helper-only or renderer-only state.
