# 008 - Gold Node Placement Consumer Parity Simulation
...
1. Add `goldNodePlacementEcho` to the tiny source fixture.
...
- Keep `goldNodePlacementEcho` as a source-data concern only.
...
Future implementation is acceptable only if renderer placement and object protokits echo mine id, seam id, resource node id, raycast hit, and source revision and the stop condition cannot pass through helper-only or renderer-only state.
