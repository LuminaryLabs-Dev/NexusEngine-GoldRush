# 007 - Mine Interaction Anchor Query Simulation
...
1. Add `mineInteractionAnchorQuery` to the tiny source fixture.
...
- Keep `mineInteractionAnchorQuery` as a source-data concern only.
...
Future implementation is acceptable only if getMineInteractionAnchor returns annotation id, hit position, normal, stance, range, and revision at named proof points and the stop condition cannot pass through helper-only or renderer-only state.
