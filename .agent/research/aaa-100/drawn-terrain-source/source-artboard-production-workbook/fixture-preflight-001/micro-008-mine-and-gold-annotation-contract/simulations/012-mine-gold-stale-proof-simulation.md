# 012 - Mine Gold Stale Proof Simulation
...
1. Add `mineGoldRevisionPolicy` to the tiny source fixture.
...
- Keep `mineGoldRevisionPolicy` as a source-data concern only.
...
Future implementation is acceptable only if source revision changes mark placement, interaction, cargo, scoring, screenshots, simulator proof, and public proof stale and the stop condition cannot pass through helper-only or renderer-only state.
