...
Make `scaleRevisionPolicy` small enough for a future implementation pass.

## Source Field

- Required field: `scaleRevisionPolicy`.
- The owning terrain source kit must define this field before consumer kits can derive behavior from it.

## Validator Case

- Fail when `scaleRevisionPolicy` is missing, non-finite, contradictory, or silently inferred by a consumer.
...
Stop if this micro-step can pass while old screenshots, caches, or public proof are reused after a map scale change.
