# 008 - Query Clamp Vs Reject Policy
...
Make `queryBoundsMode` small enough for a future implementation pass.

## Source Field

- Required field: `queryBoundsMode`.
- The owning terrain source kit must define this field before consumer kits can derive behavior from it.

## Validator Case

- Fail when `queryBoundsMode` is missing, non-finite, contradictory, or silently inferred by a consumer.
...
Stop if this micro-step can pass while debug proof passes by clamping broken points instead of exposing map boundary errors.
