# 007 - Out Of Bounds Negative Case
...
Make `boundsPolicy` small enough for a future implementation pass.

## Source Field

- Required field: `boundsPolicy`.
- The owning terrain source kit must define this field before consumer kits can derive behavior from it.

## Validator Case

- Fail when `boundsPolicy` is missing, non-finite, contradictory, or silently inferred by a consumer.
...
validator fails a fixture where outside points silently clamp as playable ground.

## Stop Condition

Stop if this micro-step can pass while players, bots, and props can exist beyond the authored map without explicit handling.
