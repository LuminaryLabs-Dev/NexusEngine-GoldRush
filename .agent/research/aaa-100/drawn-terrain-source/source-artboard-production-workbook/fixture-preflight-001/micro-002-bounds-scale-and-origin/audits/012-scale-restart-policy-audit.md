# 012 - Scale Restart Policy Audit
...
The bounds, scale, and origin pass can fake completion if `scaleRevisionPolicy` exists but consumers do not use it.

## Why It Matters

old screenshots, caches, or public proof are reused after a map scale change.
...
Do not mark this micro-step resolved until changing bounds, origin, unit scale, or cell size marks derived proof stale and the validator catches the opposite failure.
