# 001 - Cashout Site Schema Simulation

Status: planned docs-only
Parent atom: `010-cashout-and-extraction-annotation-contract`
Source field: `cashoutSites`

## Simulated Implementation

1. Add `cashoutSites` to the tiny source fixture.
...
- Keep `cashoutSites` as a source-data concern only.
...
Future implementation is acceptable only if validator proves source-owned cashout sites have id, shape, radius, role, approach lanes, state tags, and revision. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
