# 004 - Return Route Linkage Simulation
...
1. Add `cashoutReturnRoutes` to the tiny source fixture.
...
- Keep `cashoutReturnRoutes` as a source-data concern only.
...
Future implementation is acceptable only if validator proves cashout sites link back to route ids, return lanes, mine routes, retreat lanes, and fixture revision. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
