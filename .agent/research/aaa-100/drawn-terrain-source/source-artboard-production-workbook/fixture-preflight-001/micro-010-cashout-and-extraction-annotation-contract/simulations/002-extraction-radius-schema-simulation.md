# 002 - Extraction Radius Schema Simulation

Status: planned docs-only
Parent atom: `010-cashout-and-extraction-annotation-contract`
Source field: `extractionRadii`

## Simulated Implementation

1. Add `extractionRadii` to the tiny source fixture.
...
- Keep `extractionRadii` as a source-data concern only.
...
Future implementation is acceptable only if validator proves every extraction/cashout site has radius, hold radius, contest radius, safe edge, and revision. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
