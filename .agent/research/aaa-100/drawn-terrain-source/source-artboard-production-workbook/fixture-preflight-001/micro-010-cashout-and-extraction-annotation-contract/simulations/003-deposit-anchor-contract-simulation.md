# 003 - Deposit Anchor Contract Simulation
...
1. Add `depositAnchors` to the tiny source fixture.
...
- Keep `depositAnchors` as a source-data concern only.
...
Future implementation is acceptable only if validator proves deposit/cashout interaction anchors have id, position, facing, stance, input range, and revision. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
