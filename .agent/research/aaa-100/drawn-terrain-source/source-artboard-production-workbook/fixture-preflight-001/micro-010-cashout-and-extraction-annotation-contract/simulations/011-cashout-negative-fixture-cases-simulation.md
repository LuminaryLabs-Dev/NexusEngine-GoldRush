# 011 - Cashout Negative Fixture Cases Simulation

Status: planned docs-only
Parent atom: `010-cashout-and-extraction-annotation-contract`
Source field: `cashoutNegativeCases`

## Simulated Implementation

1. Add `cashoutNegativeCases` to the tiny source fixture.
...
- Keep `cashoutNegativeCases` as a source-data concern only.
...
Future implementation is acceptable only if validator fails missing site ids, duplicate anchors, invalid radii, orphan return routes, unknown tags, unreachable sites, and stale consumer echoes. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
