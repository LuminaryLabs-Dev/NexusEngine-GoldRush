# 005 - Cashout Risk And Contest Tags Simulation
...
1. Add `cashoutRiskContestTags` to the tiny source fixture.
...
- Keep `cashoutRiskContestTags` as a source-data concern only.
...
Future implementation is acceptable only if validator proves cashout sites expose risk tier, contest class, visibility class, pressure relation, and extraction value tags as closed values. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
