# 007 - Cashout Query Api Shape Simulation

Status: planned docs-only
Parent atom: `010-cashout-and-extraction-annotation-contract`
Source field: `cashoutQueryApi`

## Simulated Implementation

1. Add `cashoutQueryApi` to the tiny source fixture.
...
- Keep `cashoutQueryApi` as a source-data concern only.
...
Future implementation is acceptable only if getCashoutAt reports site id, radius band, anchor id, route link, risk tags, and revision at named proof points. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
