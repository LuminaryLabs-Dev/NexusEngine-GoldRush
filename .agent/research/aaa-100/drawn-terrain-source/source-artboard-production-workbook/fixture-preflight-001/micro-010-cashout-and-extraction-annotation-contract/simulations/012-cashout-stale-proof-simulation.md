# 012 - Cashout Stale Proof Simulation

Status: planned docs-only
Parent atom: `010-cashout-and-extraction-annotation-contract`
Source field: `cashoutRevisionPolicy`

## Simulated Implementation

1. Add `cashoutRevisionPolicy` to the tiny source fixture.
...
- Keep `cashoutRevisionPolicy` as a source-data concern only.
...
Future implementation is acceptable only if source revision changes mark renderer markers, extraction holds, receipts, scoring, screenshots, simulator proof, and public proof stale. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
