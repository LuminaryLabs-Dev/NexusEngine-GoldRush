# 010 - Receipt And Results Consumer Parity Simulation
...
1. Add `cashoutReceiptResultsEcho` to the tiny source fixture.
...
- Keep `cashoutReceiptResultsEcho` as a source-data concern only.
...
Future implementation is acceptable only if extraction receipts, scoring, replay, and results name the same source cashout annotation that accepted the carried gold. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
