# 009 - Extraction Hold Consumer Parity Simulation
...
1. Add `extractionHoldEcho` to the tiny source fixture.
...
- Keep `extractionHoldEcho` as a source-data concern only.
...
Future implementation is acceptable only if extraction hold snapshots echo source cashout id, anchor id, hold progress, contest state, cancel reason, and revision. and the stop condition cannot pass through helper-only, gameplay-only, renderer-only, scoring-only, or stale-proof state.
