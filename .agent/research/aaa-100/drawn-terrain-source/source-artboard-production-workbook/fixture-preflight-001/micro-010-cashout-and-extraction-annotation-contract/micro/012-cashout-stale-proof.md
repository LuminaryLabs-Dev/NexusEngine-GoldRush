...
Make cashout proof refresh whenever the source cashout annotation changes.

## Source Field

- Required field: `cashoutRevisionPolicy`.
...
- Fail when `cashoutRevisionPolicy` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
- All local, simulator, Playwright, and public proof artifacts must include cashout source revision and fail when stale.
...
source revision changes mark renderer markers, extraction holds, receipts, scoring, screenshots, simulator proof, and public proof stale.

## Stop Condition

Stop if source cashout changes do not force extraction proof refresh.
