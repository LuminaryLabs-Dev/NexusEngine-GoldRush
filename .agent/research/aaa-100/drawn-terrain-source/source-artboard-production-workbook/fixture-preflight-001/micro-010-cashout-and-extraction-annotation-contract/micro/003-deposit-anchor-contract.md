...
Make the actual deposit interaction point source-owned instead of implied by marker position.

## Source Field

- Required field: `depositAnchors`.
...
- Fail when `depositAnchors` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
- Interaction prompts, character alignment, hold progress, train/staging bots, receipts, replay, and results must name the same deposit anchor.
