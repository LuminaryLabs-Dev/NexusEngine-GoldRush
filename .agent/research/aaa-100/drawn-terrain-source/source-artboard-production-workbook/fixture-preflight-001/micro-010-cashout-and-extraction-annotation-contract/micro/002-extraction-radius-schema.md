...
Make extraction range, hold range, and contest range explicit source data.

## Source Field

- Required field: `extractionRadii`.
...
- Fail when `extractionRadii` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
- Hold-action, contest-state, prompt, VFX, audio, receipt, replay, and public proof consumers must echo the same radius ids and revision.
...
validator proves every extraction/cashout site has radius, hold radius, contest radius, safe edge, and revision.

## Stop Condition

Stop if extraction range is only hardcoded in the gameplay loop.
