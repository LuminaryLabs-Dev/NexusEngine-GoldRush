# 008 - Renderer Cashout Marker Parity
...
Make cashout visuals prove source identity instead of merely drawing a beacon.

## Source Field

- Required field: `rendererCashoutEcho`.
...
- Fail when `rendererCashoutEcho` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
- Every visible cashout marker, ring, beacon, prompt, and screenshot proof must carry source id and revision provenance.
...
renderer snapshots echo source cashout id, anchor id, state/readability role, radius band, and fixture revision.

## Stop Condition

Stop if marker visuals can exist without source cashout annotation ids.
