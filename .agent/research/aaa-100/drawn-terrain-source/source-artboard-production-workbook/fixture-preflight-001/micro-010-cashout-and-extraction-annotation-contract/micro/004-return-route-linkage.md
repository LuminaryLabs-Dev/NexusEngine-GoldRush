...
Make the path back to extraction/cashout authored map data instead of transient guidance math.

## Source Field

- Required field: `cashoutReturnRoutes`.
...
- Fail when `cashoutReturnRoutes` is missing, duplicated, contradictory, unversioned, non-source-owned, renderer-inferred, helper-only, or silently replaced by a consumer.
...
- Route guidance, bot routing, pressure lanes, extraction prompts, receipts, replay, and public proof must echo return route ids.
