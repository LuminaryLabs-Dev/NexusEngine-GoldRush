# 011 - Height Negative Fixture Cases Audit
...
The height sample pass can fake completion if `heightNegativeCases` exists but consumers do not use it.

## Why It Matters

validation becomes an existence check and cannot catch broken terrain source data.
...
Do not mark this micro-step resolved until validator fails missing grid, bad dimensions, non-finite values, and mismatched sample counts and the validator catches the opposite failure.
