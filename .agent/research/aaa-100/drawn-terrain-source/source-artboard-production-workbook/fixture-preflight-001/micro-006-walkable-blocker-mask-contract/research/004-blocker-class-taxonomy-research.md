# 004 - Blocker Class Taxonomy Research
...
What external and local architecture signals constrain `blockerClasses` before implementation?
...
- Walkability and blocker masks own query output for `blockerClasses`.
...
The likely fake-completion path is one generic blocked flag that cannot explain why a route failed or which player action should recover.
