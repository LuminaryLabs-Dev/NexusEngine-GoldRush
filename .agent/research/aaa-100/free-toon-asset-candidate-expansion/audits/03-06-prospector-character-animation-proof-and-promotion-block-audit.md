# Prospector Character Animation - Proof And Promotion Block Audit
...
Harden proof and promotion block for the Prospector Character Animation candidate set so free asset work moves GoldRush toward AAA toon western quality without unsafe imports or renderer-owned content.

## Architecture Boundary

`candidate source -> evidence -> candidate record -> style adaptation -> n:goldrush:prospector-character-protokits -> proof -> approval -> runtime promotion`
...
| Kit ownership can be skipped | Fast rendering/imports can hide logic in presentation code | Require n:goldrush:prospector-character-protokits descriptor before consumer use |
| Proof can be too narrow | A model preview does not prove game improvement | Require lobby spin and over-shoulder movement proof |
...
Do not implement proof and promotion block as a broad asset pass. Treat it as one gate for one candidate family. The next state should be a small evidence-backed candidate record and proof plan, not runtime content.
