# Cargo Carry Risk - Event Flow Research

Status: planned docs-only
Related packet: ../slices/13-03-cargo-carry-risk-event-flow.md
...
The owning GoldRush kit is `n:goldrush:gold-carrying`. The reusable dependency is `n:gameplay:cargo`. The future implementation should keep public API small, push detailed work into private helpers, and expose facts through events and snapshots.

## Data Implication

This slice should expose a stable `cargoCarryRisk` snapshot and emit `cargo.carried.changed`. Adjacent slices should subscribe to the event instead of reaching into private state.

## Validation Implication

Use `cargo-visual proof plus extraction-loop validator` as the initial proof route, then add a browser or simulator proof when the slice touches player-facing flow. Negative fixtures must cover this fakeout: A score number increases but the player does not feel cargo weight, value, or danger.
