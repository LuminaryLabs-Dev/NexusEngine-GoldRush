# Gold Field Spawn Map Source - Event Flow Research

Status: planned docs-only
Related packet: ../slices/08-03-gold-field-spawn-map-source-event-flow.md
...
The owning GoldRush kit is `n:goldrush:desert-world-map`. The reusable dependency is `n:world:terrain-heightfield`. The future implementation should keep public API small, push detailed work into private helpers, and expose facts through events and snapshots.

## Data Implication

This slice should expose a stable `goldFieldSpawnMapSource` snapshot and emit `world.map.revision.loaded`. Adjacent slices should subscribe to the event instead of reaching into private state.

## Validation Implication

Use `validate-terrain-heightfield plus authored fixture validator` as the initial proof route, then add a browser or simulator proof when the slice touches player-facing flow. Negative fixtures must cover this fakeout: A player appears in a desert scene but the map source cannot explain why that spawn is valid.
