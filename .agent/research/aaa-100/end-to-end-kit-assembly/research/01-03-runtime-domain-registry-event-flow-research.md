# Runtime Domain Registry - Event Flow Research

Status: planned docs-only
Related packet: ../slices/01-03-runtime-domain-registry-event-flow.md
...
The owning GoldRush kit is `n:goldrush:runtime`. The reusable dependency is `n:runtime:domain-registry`. The future implementation should keep public API small, push detailed work into private helpers, and expose facts through events and snapshots.

## Data Implication

This slice should expose a stable `runtimeDomainRegistry` snapshot and emit `runtime.kit.registry.changed`. Adjacent slices should subscribe to the event instead of reaching into private state.

## Validation Implication

Use `validate-domain-kit-contracts` as the initial proof route, then add a browser or simulator proof when the slice touches player-facing flow. Negative fixtures must cover this fakeout: A kit file exists but no runtime registry, lifecycle, or proof knows it exists.
