# Cover Combat Route - Event Flow Research

Status: planned docs-only
Related packet: ../slices/15-03-cover-combat-route-event-flow.md
...
The owning GoldRush kit is `n:goldrush:combat-route-guidance plus n:goldrush:cover-protokits`. The reusable dependency is `n:physics:query plus n:control:third-person-camera`. The future implementation should keep public API small, push detailed work into private helpers, and expose facts through events and snapshots.

## Data Implication

This slice should expose a stable `coverCombatRoute` snapshot and emit `combat.route.target.changed`. Adjacent slices should subscribe to the event instead of reaching into private state.

## Validation Implication

Use `proof:combat-route-guidance` as the initial proof route, then add a browser or simulator proof when the slice touches player-facing flow. Negative fixtures must cover this fakeout: A proof directly activates combat or cover without walking through a readable encounter path.
