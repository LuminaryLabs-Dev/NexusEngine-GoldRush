# Train Boarding Party Sync - Event Flow

Status: planned docs-only
Slice: 07 Train Boarding Party Sync
Domain: network/scene
Scene/site: site.loading-yard
Generic kit: n:network:party-room
GoldRush kit: n:goldrush:party-boarding-sync
...
Gate train departure on real party readiness while preserving single-player staging speed.

## Atomic Substeps

1. Confirm this slice is owned by `n:goldrush:party-boarding-sync` and not by renderer-only logic.
...
- Event: `party.boarding.ready.changed`
- Snapshot: `trainBoardingPartySync`
- Reset rule: clear transient scene state, retain durable receipts only when the match lifecycle owns them.

## Validator Seed

`proof:peer-party-boarding and proof:peer-party-disconnect`

## Human-View Proof Seed

two-browser party boards, disconnect policy releases remaining roster, and no stale peer blocks the train

## Known Fakeout

The leader launches but member readiness, disconnect, or late join state cannot be inspected.
