# Title Audio Entry - Event Flow

Status: planned docs-only
Slice: 03 Title Audio Entry
Domain: presentation/audio
Scene/site: site.start
Generic kit: n:audio:cue-state
...
Start the game with GoldRush identity, safe audio cues, settings affordance, and a clear first action.

## Atomic Substeps

1. Confirm this slice is owned by `n:goldrush:music-and-stingers` and not by renderer-only logic.
...
- Event: `title.entry.started`
- Snapshot: `titleAudioEntry`
- Reset rule: clear transient scene state, retain durable receipts only when the match lifecycle owns them.

## Validator Seed

`validate-audio-cue-approval-plan`

## Human-View Proof Seed

title screen plays approved or semantic fallback cue and exposes no sustained hum or hidden debug dependency

## Known Fakeout

A sound plays but is not tied to semantic cue state or asset approval status.
