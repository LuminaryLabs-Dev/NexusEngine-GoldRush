# Train Loading Sequence - Event Flow

Status: planned docs-only
Slice: 06 Train Loading Sequence
Domain: scene/transition/audio
...
Make the train arrive, open, board, lock player, depart, and hand off to the run as one reliable sequence.

## Atomic Substeps

1. Confirm this slice is owned by `n:goldrush:train-loading` and not by renderer-only logic.
...
- Event: `train.sequence.beat.changed`
- Snapshot: `trainLoadingSequence`
...
browser proof shows arrival, open door, board action, locked rider, departure, and field handoff

## Known Fakeout

The scene changes to the field without the player performing a readable train interaction.
