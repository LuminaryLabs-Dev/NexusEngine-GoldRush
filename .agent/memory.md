...
- Authored terrain source fixtures now expose `sampleGround` as the source-owned footing contract: world-space normals, slope degrees, slope class taxonomy, walkable thresholds, movement echoes, placement echoes, and stale-proof policy all derive from one terrain sample result.
