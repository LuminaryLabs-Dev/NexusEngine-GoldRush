# Simulation 002 - Current state baseline

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/002-current-state-baseline.md
Domain: governance
Current matrix state: planned
Owning domain service: governance ledger
Candidate generic kit: n:runtime:validation
Candidate GoldRush kit: n:goldrush:reality-status
Audit gate: Compare every new pass against baseline

## Intent

Simulate what implementation of **Current state baseline** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **governance ledger** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Current state baseline; keep tuning/config minimal.
...
- Confirm owner: bind the problem to governance ledger, not a loose renderer/app helper.
...
The main risk is documentation that sounds complete but never blocks bad implementation claims.

## Player-View Simulation

The player does not see this directly, but it prevents the project from calling a prototype a finished game.

For this specific step, the player-facing question is: does **Current state baseline** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the governance contract without browser state.
- Human-view proof: required if Current state baseline affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Current state baseline claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Compare every new pass against baseline**.
