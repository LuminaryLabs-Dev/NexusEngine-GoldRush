# Simulation 075 - Threat AI and bots

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/075-threat-ai-bots.md
...
Audit gate: Single-player staging has enemies

## Intent

Simulate what implementation of **Threat AI/bots** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Threat AI/bots; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Threat AI/bots** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Threat AI/bots affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Threat AI/bots claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Single-player staging has enemies**.
