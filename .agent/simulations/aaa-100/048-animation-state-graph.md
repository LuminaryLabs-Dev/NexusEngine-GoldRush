# Simulation 048 - Animation state graph

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/048-animation-state-graph.md
Domain: animation
...
Audit gate: Every loop action has animation state

## Intent

Simulate what implementation of **Animation state graph** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Animation state graph; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Animation state graph** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the animation contract without browser state.
- Human-view proof: required if Animation state graph affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Animation state graph claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Every loop action has animation state**.
