# Simulation 053 - Controller and accessibility input

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/053-controller-accessibility-input.md
Domain: control/accessibility
Current matrix state: planned
Owning domain service: player control and character feel
Candidate generic kit: n:control:character-movement
Candidate GoldRush kit: n:goldrush:prospector-movement
Audit gate: Core loop does not depend on one input style

## Intent

Simulate what implementation of **Controller/accessibility input** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **player control and character feel** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Controller/accessibility input; keep tuning/config minimal.
...
- Confirm owner: bind the problem to player control and character feel, not a loose renderer/app helper.
...
The main risk is two systems trying to move camera, player, or animation state in the same frame.

## Player-View Simulation

The player should believe they are controlling a character, not pushing a placeholder rig through a camera test.

For this specific step, the player-facing question is: does **Controller/accessibility input** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the control/accessibility contract without browser state.
- Human-view proof: required if Controller/accessibility input affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Controller/accessibility input claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:prospector-movement, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Core loop does not depend on one input style**.
