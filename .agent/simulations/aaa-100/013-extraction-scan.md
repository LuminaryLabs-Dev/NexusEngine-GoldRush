# Simulation 013 - Extraction scan

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/013-extraction-scan.md
Domain: research
...
Audit gate: Gold value creates risk decisions

## Intent

Simulate what implementation of **Extraction scan** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Extraction scan; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Extraction scan** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Extraction scan affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Extraction scan claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Gold value creates risk decisions**.
