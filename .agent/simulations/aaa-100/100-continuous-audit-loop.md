# Simulation 100 - Continuous audit loop

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/100-continuous-audit-loop.md
Domain: production
...
Audit gate: Project keeps improving after this plan

## Intent

Simulate what implementation of **Continuous audit loop** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **production and release gates** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Continuous audit loop; keep tuning/config minimal.
...
- Confirm owner: bind the problem to production and release gates, not a loose renderer/app helper.
...
The main risk is making gates so broad that they are skipped during fast iteration.

## Player-View Simulation

Players get fewer broken public builds because deploy claims stay tied to proof.

For this specific step, the player-facing question is: does **Continuous audit loop** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the production contract without browser state.
- Human-view proof: required if Continuous audit loop affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Continuous audit loop claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Project keeps improving after this plan**.
