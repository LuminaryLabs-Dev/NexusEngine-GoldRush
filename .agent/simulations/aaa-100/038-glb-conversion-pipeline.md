# Simulation 038 - GLB conversion pipeline

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/038-glb-conversion-pipeline.md
Domain: content
...
Audit gate: Models enter as usable game assets

## Intent

Simulate what implementation of **GLB conversion pipeline** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for GLB conversion pipeline; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **GLB conversion pipeline** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the content contract without browser state.
- Human-view proof: required if GLB conversion pipeline affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if GLB conversion pipeline claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Models enter as usable game assets**.
