# Simulation 015 - Player persona matrix

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/015-player-persona-matrix.md
Domain: product
...
Audit gate: Every persona has a proof path

## Intent

Simulate what implementation of **Player persona matrix** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Player persona matrix; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Player persona matrix** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the product contract without browser state.
- Human-view proof: required if Player persona matrix affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Player persona matrix claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Every persona has a proof path**.
