# Simulation 031 - Town and mine camp layout

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/031-town-mine-camp-layout.md
Domain: world/art
...
Audit gate: Spaces support interaction

## Intent

Simulate what implementation of **Town/mine camp layout** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Town/mine camp layout; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Town/mine camp layout** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the world/art contract without browser state.
- Human-view proof: required if Town/mine camp layout affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Town/mine camp layout claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Spaces support interaction**.
