# Simulation 023 - Height and mask data model

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/023-height-mask-data-model.md
Domain: world
Current matrix state: active
Owning domain service: world and map source
Candidate generic kit: n:world:terrain-heightfield
Candidate GoldRush kit: n:goldrush:desert-world-map
Audit gate: Visual and gameplay use same data

## Intent

Simulate what implementation of **Height/mask data model** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **world and map source** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Height/mask data model; keep tuning/config minimal.
...
- Confirm owner: bind the problem to world and map source, not a loose renderer/app helper.
...
The main risk is letting visual terrain, collider terrain, and gameplay placement become three different maps.

## Player-View Simulation

The player should feel a large authored desert with readable routes, landmarks, gold risk, and extraction tension.

For this specific step, the player-facing question is: does **Height/mask data model** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the world contract without browser state.
- Human-view proof: required if Height/mask data model affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Height/mask data model claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:desert-world-map, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Visual and gameplay use same data**.
