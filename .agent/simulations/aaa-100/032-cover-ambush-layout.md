# Simulation 032 - Cover and ambush layout

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/032-cover-ambush-layout.md
Domain: combat/world
...
Audit gate: Combat has readable counterplay

## Intent

Simulate what implementation of **Cover and ambush layout** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Cover and ambush layout; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Cover and ambush layout** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the combat/world contract without browser state.
- Human-view proof: required if Cover and ambush layout affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Cover and ambush layout claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Combat has readable counterplay**.
