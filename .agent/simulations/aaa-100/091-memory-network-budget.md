# Simulation 091 - Memory and network budget

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/091-memory-network-budget.md
Domain: performance/network
...
Audit gate: 60-player mode has limits

## Intent

Simulate what implementation of **Memory/network budget** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Memory/network budget; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Memory/network budget** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the performance/network contract without browser state.
- Human-view proof: required if Memory/network budget affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Memory/network budget claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **60-player mode has limits**.
