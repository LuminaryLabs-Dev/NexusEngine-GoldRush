# Simulation 014 - Visual target packet

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/014-visual-target-packet.md
Domain: art direction
Current matrix state: planned
Owning domain service: render and art direction
Candidate generic kit: n:render:three-scene
Candidate GoldRush kit: n:goldrush:3d-scene-renderer
Audit gate: Prototype visuals cannot pass as final

## Intent

Simulate what implementation of **Visual target packet** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **render and art direction** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Visual target packet; keep tuning/config minimal.
...
- Confirm owner: bind the problem to render and art direction, not a loose renderer/app helper.
...
- materialRoles
- lodState
- cameraFacingReadability
- visualBudget

## Likely Bottleneck

The main risk is making the scene prettier while preserving primitive silhouettes and unclear affordances.

## Player-View Simulation

The player should read shape, scale, mood, and interaction without debug overlays.

For this specific step, the player-facing question is: does **Visual target packet** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the art direction contract without browser state.
- Human-view proof: required if Visual target packet affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Visual target packet claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:3d-scene-renderer, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Prototype visuals cannot pass as final**.
