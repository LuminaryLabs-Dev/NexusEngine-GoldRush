# Simulation 085 - Matchmaker and private rooms

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/085-matchmaker-private-rooms.md
Domain: network/product
...
Audit gate: Testing and play sessions can start

## Intent

Simulate what implementation of **Matchmaker/private rooms** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Matchmaker/private rooms; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Matchmaker/private rooms** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the network/product contract without browser state.
- Human-view proof: required if Matchmaker/private rooms affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Matchmaker/private rooms claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Testing and play sessions can start**.
