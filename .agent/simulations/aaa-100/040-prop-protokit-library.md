# Simulation 040 - Prop protokit library

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/040-prop-protokit-library.md
Domain: content
...
Audit gate: Every object is kit-owned

## Intent

Simulate what implementation of **Prop protokit library** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Prop protokit library; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Prop protokit library** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Prop protokit library affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Prop protokit library claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Every object is kit-owned**.
