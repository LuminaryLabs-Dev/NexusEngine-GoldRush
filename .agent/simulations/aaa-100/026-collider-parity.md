# Simulation 026 - Collider parity

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/026-collider-parity.md
Domain: physics
Current matrix state: active
Owning domain service: physics and collider parity
Candidate generic kit: n:physics:collider
Candidate GoldRush kit: n:goldrush:terrain-physics
Audit gate: Player never floats or sinks

## Intent

Simulate what implementation of **Collider parity** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **physics and collider parity** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Collider parity; keep tuning/config minimal.
...
- Confirm owner: bind the problem to physics and collider parity, not a loose renderer/app helper.
...
The main risk is adding physics backend complexity before terrain source parity is locked.

## Player-View Simulation

The player should never float, sink, pulse, slide through mountains, or collide with invisible wrong shapes.

For this specific step, the player-facing question is: does **Collider parity** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the physics contract without browser state.
- Human-view proof: required if Collider parity affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Collider parity claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:terrain-physics, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Player never floats or sinks**.
