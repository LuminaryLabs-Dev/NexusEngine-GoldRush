# Simulation 089 - Playwright human-view suite

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/089-playwright-human-view-suite.md
Domain: validation
Current matrix state: planned
Owning domain service: proof and validation
Candidate generic kit: n:runtime:validation
Candidate GoldRush kit: n:goldrush:reality-status
Audit gate: Player view remains primary

## Intent

Simulate what implementation of **Playwright human-view suite** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **proof and validation** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Playwright human-view suite; keep tuning/config minimal.
...
- Confirm owner: bind the problem to proof and validation, not a loose renderer/app helper.
...
The main risk is proof that validates a helper path instead of the natural player action.

## Player-View Simulation

A real player-view proof catches bugs that state receipts alone miss.

For this specific step, the player-facing question is: does **Playwright human-view suite** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the validation contract without browser state.
- Human-view proof: required if Playwright human-view suite affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Playwright human-view suite claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Player view remains primary**.
