# Simulation 011 - Game-engine feature scan

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/011-game-engine-feature-scan.md
Domain: research
Current matrix state: planned
Owning domain service: research to implementation bridge
Candidate generic kit: n:runtime:validation
Candidate GoldRush kit: n:goldrush:reality-status
Audit gate: Missing production surfaces are named

## Intent

Simulate what implementation of **Game-engine feature scan** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **research to implementation bridge** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Game-engine feature scan; keep tuning/config minimal.
...
- Confirm owner: bind the problem to research to implementation bridge, not a loose renderer/app helper.
...
The main risk is copying genre language without converting it into GoldRush-specific kit work.

## Player-View Simulation

Research only matters if it becomes a clearer game decision or proof gate.

For this specific step, the player-facing question is: does **Game-engine feature scan** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the research contract without browser state.
- Human-view proof: required if Game-engine feature scan affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Game-engine feature scan claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:reality-status, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Missing production surfaces are named**.
