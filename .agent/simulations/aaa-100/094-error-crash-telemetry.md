# Simulation 094 - Error and crash telemetry

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/094-error-crash-telemetry.md
Domain: runtime
...
Audit gate: Failures are diagnosable

## Intent

Simulate what implementation of **Error/crash telemetry** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Error/crash telemetry; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Error/crash telemetry** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the runtime contract without browser state.
- Human-view proof: required if Error/crash telemetry affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Error/crash telemetry claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Failures are diagnosable**.
