# Simulation 041 - Rock and plant and clutter kits

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/041-rock-plant-clutter-kits.md
...
Audit gate: Clutter supports scale, not noise

## Intent

Simulate what implementation of **Rock/plant/clutter kits** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Rock/plant/clutter kits; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Rock/plant/clutter kits** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the content/world contract without browser state.
- Human-view proof: required if Rock/plant/clutter kits affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Rock/plant/clutter kits claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Clutter supports scale, not noise**.
