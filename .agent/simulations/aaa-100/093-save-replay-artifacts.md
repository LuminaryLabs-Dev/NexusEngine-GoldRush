# Simulation 093 - Save and replay artifacts

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/093-save-replay-artifacts.md
Domain: runtime
...
Audit gate: Bugs can be replayed

## Intent

Simulate what implementation of **Save/replay artifacts** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Save/replay artifacts; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Save/replay artifacts** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Save/replay artifacts affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Save/replay artifacts claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Bugs can be replayed**.
