# Simulation 029 - Path and rail splines

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/029-path-rail-splines.md
Domain: world
...
Audit gate: Train and walking paths agree

## Intent

Simulate what implementation of **Path and rail splines** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Path and rail splines; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Path and rail splines** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Path and rail splines affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Path and rail splines claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Train and walking paths agree**.
