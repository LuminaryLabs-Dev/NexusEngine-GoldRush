# Simulation 088 - NexusSimulator suite

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/088-nexussimulator-suite.md
Domain: validation
...
Audit gate: Simulation catches regressions

## Intent

Simulate what implementation of **NexusSimulator suite** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for NexusSimulator suite; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **NexusSimulator suite** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if NexusSimulator suite affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if NexusSimulator suite claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Simulation catches regressions**.
