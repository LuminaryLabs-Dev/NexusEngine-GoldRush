# Simulation 078 - 60-player partition contracts

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/078-60-player-partition-contracts.md
Domain: network
...
Audit gate: 60-player scale is testable

## Intent

Simulate what implementation of **60-player partition contracts** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **network and multiplayer scale** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for 60-player partition contracts; keep tuning/config minimal.
...
- Confirm owner: bind the problem to network and multiplayer scale, not a loose renderer/app helper.
...
The main risk is labeling local simulation as live 60-player networking proof.

## Player-View Simulation

The player should experience stable party start, fair state sync, and credible 60-player pressure.

For this specific step, the player-facing question is: does **60-player partition contracts** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the network contract without browser state.
- Human-view proof: required if 60-player partition contracts affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if 60-player partition contracts claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:room-orchestration, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **60-player scale is testable**.
