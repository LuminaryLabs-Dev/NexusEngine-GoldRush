# Simulation 067 - Reward and score receipts

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/067-reward-score-receipts.md
Domain: match
...
Audit gate: Results are explainable

## Intent

Simulate what implementation of **Reward/score receipts** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Reward/score receipts; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Reward/score receipts** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the match contract without browser state.
- Human-view proof: required if Reward/score receipts affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Reward/score receipts claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Results are explainable**.
