# Simulation 073 - Health and downed and revive

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/073-health-downed-revive.md
Domain: combat/team
Current matrix state: planned
Owning domain service: combat pressure and counterplay
Candidate generic kit: n:gameplay:combat-pressure
Candidate GoldRush kit: n:goldrush:ambush-pressure
Audit gate: Squads have teamwork

## Intent

Simulate what implementation of **Health/downed/revive** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
2. Confirm the owning domain service is still **combat pressure and counterplay** and that no newer packet replaced this step.
3. Identify the smallest public API surface needed for Health/downed/revive; keep tuning/config minimal.
...
- Confirm owner: bind the problem to combat pressure and counterplay, not a loose renderer/app helper.
...
The main risk is combat data without hit feedback, readable cover, or fair interruption states.

## Player-View Simulation

The player should read threat direction, cover value, weapon role, and extraction risk under pressure.

For this specific step, the player-facing question is: does **Health/downed/revive** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the combat/team contract without browser state.
- Human-view proof: required if Health/downed/revive affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Health/downed/revive claims 20, 50, 60, or 100 player scale.
...
- If implementation finds a better owner than n:goldrush:ambush-pressure, update this packet instead of layering duplicate kits.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Squads have teamwork**.
