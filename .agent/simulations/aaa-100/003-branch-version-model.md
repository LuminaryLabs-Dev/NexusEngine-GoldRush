# Simulation 003 - Branch and version model

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/003-branch-version-model.md
Domain: governance
...
Audit gate: Simulate bad deploy rollback

## Intent

Simulate what implementation of **Branch/version model** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Branch/version model; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Branch/version model** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Branch/version model affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Branch/version model claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Simulate bad deploy rollback**.
