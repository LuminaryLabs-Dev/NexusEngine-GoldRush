# Simulation 034 - Map streaming budget

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/034-map-streaming-budget.md
Domain: performance
...
Audit gate: 60-player view remains stable

## Intent

Simulate what implementation of **Map streaming budget** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Map streaming budget; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Map streaming budget** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Map streaming budget affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Map streaming budget claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **60-player view remains stable**.
