# Simulation 083 - Sanity and anti-cheat boundaries
...
Simulate what implementation of **Sanity/anti-cheat boundaries** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Sanity/anti-cheat boundaries; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Sanity/anti-cheat boundaries** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the network/security contract without browser state.
- Human-view proof: required if Sanity/anti-cheat boundaries affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Sanity/anti-cheat boundaries claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Public play is not trivial to spoof**.
