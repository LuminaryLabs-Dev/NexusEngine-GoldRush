# Simulation 058 - Gold economy

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/058-gold-economy.md
Domain: gameplay
...
Audit gate: Gold creates extraction choices

## Intent

Simulate what implementation of **Gold economy** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Gold economy; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Gold economy** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?
...
- Human-view proof: required if Gold economy affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Gold economy claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Gold creates extraction choices**.
