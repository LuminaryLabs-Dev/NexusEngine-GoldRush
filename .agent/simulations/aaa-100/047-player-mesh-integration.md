# Simulation 047 - Player mesh integration

Status: docs-only dry-run
Roadmap packet: .agent/research/aaa-100/047-player-mesh-integration.md
Domain: character
...
Audit gate: Player no longer reads as placeholder

## Intent

Simulate what implementation of **Player mesh integration** would be like before runtime work starts. This packet is not permission to code; it is a pre-implementation rehearsal that should expose where the step could fake progress, break deploys, or contradict the AAA GoldRush target.
...
3. Identify the smallest public API surface needed for Player mesh integration; keep tuning/config minimal.
...
For this specific step, the player-facing question is: does **Player mesh integration** make the title -> lobby -> train -> gold field -> mine -> carry -> pressure -> extract -> results loop more understandable, tactile, scalable, or reliable?

## Validator And Proof Plan

- Minimum CLI proof: add or reuse the closest validator that can assert the character contract without browser state.
- Human-view proof: required if Player mesh integration affects camera, motion, terrain, interaction, combat, UI, audio, or visible content.
- Simulator proof: required if Player mesh integration claims 20, 50, 60, or 100 player scale.
...
Do not mark this step resolved because a state object exists. Mark it resolved only when the audit gate passes: **Player no longer reads as placeholder**.
