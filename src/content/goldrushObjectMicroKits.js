const FAMILY_BLUEPRINTS = [
  ...
  { family: "mining.gold-fleck", count: 72, geometryRole: "gold-fleck", materialRole: "gold", role: "reward-readability", zone: "gold-field", placementRole: "support", archetypes: ["nugget", "vein-chip", "pan-glint"] },
  { family: "mining.ore-chip", count: 90, geometryRole: "ore-chip", materialRole: "ore", role: "reward-readability", zone: "mine-pad", placementRole: "support", archetypes: ["tailings", "ore-slate", "dark-seam"] },
  { family: "gold.seam-vein", count: 42, geometryRole: "gold-seam", materialRole: "gold", role: "reward-readability", zone: "goldSeamZone", placementRole: "landmark", archetypes: ["embedded-vein", "rim-vein", "claim-seam"] },
  { family: "gold.tailings-line", count: 64, geometryRole: "tailings-pile", materialRole: "ore", role: "reward-readability", zone: "goldSeamZone", placementRole: "support", archetypes: ["tailings-row", "pan-wash", "ore-spill"] },
  ...
  { family: "mine.tailings-pile", count: 18, geometryRole: "tailings-pile", materialRole: "ore", role: "reward-readability", zone: "mineCampZone", placementRole: "support", archetypes: ["ore-spill", "tailings-mound", "dump-pile"] },
...
export function createGoldRushObjectMicroKits({ terrain, environmentSpace = null }) {
  ...
  for (const blueprint of FAMILY_BLUEPRINTS) {
    for (let index = 0; index < blueprint.count; index += 1) {
      ...
      const placement = createPlacementMetadata({ blueprint, seed, index, environmentSpace });
      const visual = createVisualMetadata({ blueprint, archetype, seed });
      const transform = {
        x: Number(position.x.toFixed(3)),
        y: Number((0.19 + position.elevation).toFixed(3)),
      ...
      kits.push({
        id: `goldrush.micro.${blueprint.family}.${String(index + 1).padStart(4, "0")}`,
...
function positionForSeed(seed, terrain, family) {
  ...
  if (family.startsWith("mining.") || family.startsWith("rail.")) {
    x = -7.2 + (((seed >>> 4) % 100) / 100 - 0.5) * 8.8;
    z = 5.3 + (((seed >>> 12) % 100) / 100 - 0.5) * 5.6;
  }

  if (family.startsWith("gold.")) {
    const t = (seed % 10000) / 10000;
    x = -10.5 + t * 12.5;
    z = 6.2 + Math.sin(t * Math.PI * 2) * 1.2 + (((seed >>> 9) % 100) / 100 - 0.5) * 0.65;
  }
...
function createPlacementMetadata({ blueprint, seed, index, environmentSpace }) {
